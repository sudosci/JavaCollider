...
public class Bus implements Constants {
	...
	public static Bus control(Server server, int numChannels) {
		...
		if (alloc == -1) {
			Server.getPrintStream().println("Bus.control: failed to get a control bus allocated. " + "numChannels: "
					+ numChannels + "; server: " + server.getName());
			return null;
		} else {
	...
	public static Bus audio(Server server, int numChannels) {
		...
		if (alloc == -1) {
			Server.getPrintStream().println("Bus.audio: failed to get a audio bus allocated. " + "numChannels: "
					+ numChannels + "; server: " + server.getName());
			return null;
		} else {
	...
	public OSCMessage fillMsg(int offset, int numChans, float value) {
		return new OSCMessage("/c_fill",
				new Object[] { new Integer(getIndex() + offset), new Integer(numChans), new Float(value) });
	}
	...
	public void get(GetCompletionAction action) throws IOException {
		get(0, action);
	}

	public void get(int offset, GetCompletionAction action) throws IOException {
	 ...
	 * @warning has not been tested
	 */
	public void get(final int[] offsets, final GetCompletionAction action) throws IOException {
		...
		final OSCResponderNode resp = new OSCResponderNode(getServer(), "/c_set", new OSCResponderNode.Action() {
			public void respond(OSCResponderNode r, OSCMessage msg, long time) {
				...
				for (int i = 0, j = 0; i < numVals; i++, j += 2) {
					if (((Number) msg.getArg(j)).intValue() != idx + offsets[i])
						return;
				}
	 ...
	 * @warning has not been tested
	 */
	public void getn(final int[] offsets, final int[] numChans, final GetCompletionAction action) throws IOException {
		final int numEntries = offsets.length;
		...
		final OSCResponderNode resp = new OSCResponderNode(getServer(), "/c_setn", new OSCResponderNode.Action() {
			public void respond(OSCResponderNode r, OSCMessage msg, long time) {
				...
				for (int i = 0, j = 0; j < numArgs; i++) {
					...
					if (((Number) msg.getArg(j++)).intValue() != idx + offsets[i])
						return;
					if (((Number) msg.getArg(j++)).intValue() != nc)
						return;
					numVals += nc;
					j += nc;
				}
	...
	public OSCMessage getnMsg(int offset, int numChans) {
		return new OSCMessage("/c_getn", new Object[] { new Integer(getIndex() + offset), new Integer(numChans) });
	}
	...
	public boolean equals(Object o) {
		if (o instanceof Bus) {
			...
			return (this.getIndex() == aBus.getIndex() && this.getNumChannels() == aBus.getNumChannels()
					&& this.getRate() == aBus.getRate() && this.getServer() == aBus.getServer());
		} else {
			return false;
		}
	}
