...
/**
 * The client side represenation of a unit generator. Unlike SClang, there are
 * no subclasses for all the different ugens (few exceptions, such as
 * <code>Control</code>), therefore to construct a ugen graph requires you to
 * make successive calls to the static constructor methods provided by this
 * class. See <code>SynthDef</code> and <code>JColliderDemo</code> for examples
 * of how to construct a graph.
 * <p>
 * When you use the static constructor methods of <code>UGen</code>, the class
 * relies on the UGen database to be loaded into <code>UGenInfo</code>. Please
 * see the <code>UGenInfo</code> doc for details about this database.
 * <p>
 * While some methods like <code>array( ... )</code> would better fit into the
 * <code>GraphElemArray</code> class, they have been included here, simply
 * because it's shorter to write <code>UGen.array( ... )</code> and it gives
 * your code a more homogeneous and pathological look.
 * <p>
 * To access a single output of a multi-output UGen, use either the
 * <code>getOutput</code> or <code>getChannel</code> method. To create controls,
 * use the separate <code>Control</code> class.
 *
 * @warning no rate checking is performed along the inputs
 *
 * @todo a keyword constructor variant has been removed because it wasn't too
 *       convincing (well, keywords is just a concept totally unknown to java).
 *       however to make constructor calls shorter and more readable, some sort
 *       of keyword constructor should be re-invented.
 *
...
public class UGen implements Constants, GraphElem {
	...
	/**
	 * This constructor is used by <code>SynthDef</code> when building the UGen from
	 * a def file. Do not use this method yourself.
	 */
	...
	/**
	 * Returns an array of the rates at which the individual output channels run. I
	 * know of no UGen any of whose outputs have a rate different from the UGen
	 * rate.
	 */
	...
	/**
	 * Returns the rate of one of the UGens output channels. I know of no UGen any
	 * of whose outputs have a rate different from the UGen rate.
	 */
	...
	/**
	 * Returns the UGen's input at the given index. Note that for UGens constructed
	 * with a variable length array argument, this argument is expanded to separate
	 * individual arguments. For example for <code>Out</code>, each element of the
	 * <code>channelsArray</code> argument is a separate input with a separate index
	 * (as you would expect from the binary compiled synth def format).
	 */
	...
	/**
	 * Returns the so-called special index of the UGen. This is kind of an extra
	 * internal static parameter. Special indices are used by
	 * <code>BinaryOpUGen</code> and <code>UnaryOpUGen</code> to specify the
	 * mathematical operator, and by <code>Control</code> to specify the index in
	 * the control name table. Note that this index is invalid for
	 * <code>Control</code> until the graph has been constructed by
	 * <code>SynthDef</code>.
	 */
	...
	/**
	 * This is called by <code>SynthDef</code> when assembling controls. Do not call
	 * this method directly. To create unary and binary operator ugens, simply
	 * construct them with their operator name and the ugen factory will set the
	 * right index itself. So to create a multiplying binary ugen, you will call:
	 * 
	 * <pre>
	...
	/**
	 * Returns the number of input arguments. Arrays are expanded, so an
	 * <code>Out</code> UGen with three input channels will report <code>4</code>
	 * (three inputs plus bus parameter).
	 */
	 ...
	 * &quot;BinaryOpUGen&quot; and not their operators. To get the operator names
	...
	/**
	 * Constructs a new audio rate UGen with zero arguments and a required explict
	 * output channel number. This method should only be used for UGens that require
	 * the explicit setting of the number of output channels, such as
	 * <code>LocalIn.ar</code>.
	 * <p>
	 * This method can also be used for UGens with more than zero arguments, if the
	 * info dictionary contains default values for the missing arguments. An example
	 * is <code>BufRd</code> which actually has four arguments for bufNum, phase,
	 * loop and interolation, but specifies defaults for all of them (bufNum 0,
	 * phase zero, loop 1, interpolation 2).
	 *
	 * @param name
	 *            either ugen class name or alias name (for unary/binary op)
	 * @param numChannels
	 *            number of output channels to create
	 *
	 * @throws IllegalArgumentException
	 *             if the UGen cannot be found in the info dictionary, if the rate
	 *             is not allowed for the ugen
	 * @throws IllegalStateException
	 *             if the UGens requires arguments which have not been provided and
	 ...
	 * This method can also be used for UGens with more than zero arguments, if the
	 * info dictionary contains default values for the missing arguments. An example
	 * is <code>SinOsc</code> which actually has two arguments for frequency and
	 * phase, but both of them have default values (440 hertz, zero degrees phase).
	 *
	 * @param name
	 *            either ugen class name or alias name (for unary/binary op)
	 *
	 * @throws IllegalArgumentException
	 *             if the UGen cannot be found in the info dictionary, if the rate
	 *             is not allowed for the ugen, or if the UGen requires the explicit
	 *             specification of the number of output channels
	 * @throws IllegalStateException
	 *             if the UGens requires arguments which have not been provided and
	 *             have no default values
	 */
	...
	/**
	 * Constructs a new audio rate UGen with one given argument and an explicit
	 * number of output channels. This method should only be used for UGens that
	 * require the explicit setting of the number of output channels, such as
	 * <code>PanAz.ar</code>.
	 * <p>
	 * This method can also be used for UGens with more than one argument (e.g.
	 * <code>PanAz</code>), if the info dictionary contains default values for the
	 * missing arguments. For example, <code>PanAz</code> has five arguments, the
	 * first one being the input signal which needs to specified, but the other ones
	 * having default values (pos 0.0, level 1.0, width 2.0, orientation 0.5).
	 *
	 * @param name
	 *            either ugen class name or alias name (for unary/binary op)
	 * @param numChannels
	 ...
	 * <p>
	 * This method can also be used for UGens with more than one argument (e.g.
	 * <code>Osc</code>), if the info dictionary contains default values for the
	 * missing arguments.
	 *
	 * @param name
	 *            either ugen class name or alias name (for unary/binary op)
	 ...
	 * equivalent to <code>new Constant( value )</code>.
	 */
	 ...
	 * which in turn can be used for multichannel expansion when passed as an
	 * argument to one of the UGen constructor methods
	 *
	 * @param g1
	 *            first graph element such as a <code>UGen</code>,
	 *            <code>Constant</code>, <code>UGenChannel</code> or
	 *            <code>GraphElemArray</code>
	 * @param g2
	 *            second graph element
	...
	/**
	 * Assembles three graph elements (such as UGens, or Constants) into one array
	 * which in turn can be used for multichannel expansion when passed as an
	 * argument to one of the UGen constructor methods
	 *
	 * @param g1
	...
	/**
	 * Assembles four graph elements (such as UGens, or Constants) into one array
	 * which in turn can be used for multichannel expansion when passed as an
	 * argument to one of the UGen constructor methods
	 *
	 * @param g1
	...
	/**
	 * Assembles five graph elements (such as UGens, or Constants) into one array
	 * which in turn can be used for multichannel expansion when passed as an
	 * argument to one of the UGen constructor methods
	 *
	 * @param g1
