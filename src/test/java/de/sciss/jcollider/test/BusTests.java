...
public class BusTests {
	...
	public BusTests() {
		System.out.println("Testing bus behaviour.\nAssuming scsynth is" + "running on udp port 57110...\n");
	...
	protected void test(float val1, float val2, String name) {
		if (val1 != val2)
			throw new AssertionError(name + " (" + val1 + " != " + val2 + ")");
	}
	...
	protected void iter(final int iter) throws IOException {
		...
		if (iter < 333) {
			...
			for (int i = 0, off = 0; i < numIntervals; i++) {
				// System.out.println( "i = " + i + "; off " + off + "; numInt = " +
				// numIntervals + "; numCh = " + b.getNumChannels() );
