...
/**
 * Static example generator to compare with SC examples found in the
 * from-SC2.rtf file
 *
...
public abstract class DemoDefs {
	public static List<SynthDef> create() {
		...
		f = UGen.kr("midicps",
				UGen.kr("MulAdd", UGen.kr("LFSaw", UGen.ir(0.4f), UGen.ir(0)), UGen.ir(24),
						UGen.kr("MulAdd", UGen.kr("LFSaw", UGen.array(UGen.ir(8), UGen.ir(7.23f)), UGen.ir(0)),
								UGen.ir(3), UGen.ir(80))));
		g = UGen.ar("CombN", UGen.ar("*", UGen.ar("SinOsc", f, UGen.ir(0)), UGen.ir(0.04f)), UGen.ir(0.2f),
				UGen.ir(0.2f), UGen.ir(4));
		...
		f = UGen.ar("CombL",
				UGen.ar("RLPF",
						UGen.ar("*",
								UGen.ar("LFPulse",
										UGen.kr("MulAdd", UGen.kr("FSinOsc", UGen.ir(0.05f), UGen.ir(0)), UGen.ir(80),
												UGen.ir(160)),
										UGen.ir(0), UGen.ir(0.4f)),
								UGen.ir(0.05f)),
						UGen.kr("MulAdd", UGen.kr("FSinOsc", UGen.array(UGen.ir(0.6f), UGen.ir(0.7f)), UGen.ir(0)),
								UGen.ir(3600), UGen.ir(4000)),
						UGen.ir(0.2f)),
				UGen.ir(0.3f), UGen.array(UGen.ir(0.2f), UGen.ir(0.25f)), UGen.ir(2));
		def = new SynthDef("JPulseModulation", UGen.ar("Out", UGen.ir(0), f));
		result.add(def);

		// MotoRev
		f = UGen.ar("clip2",
				UGen.ar("RLPF", UGen.ar("LFPulse",
						UGen.kr("MulAdd", UGen.kr("SinOsc", UGen.ir(0.2f), UGen.ir(0)), UGen.ir(10), UGen.ir(21)),
						UGen.ir(0.1f)), UGen.ir(100), UGen.ir(0.1f)),
				UGen.ir(0.4f));
		def = new SynthDef("JMotoRev", UGen.ar("Out", UGen.ir(0), UGen.ar("Pan2", f)));
		result.add(def);

		// Scratchy
		// IMPORTANT NOTE : bringing one instance of BrownNoise together with a mul
		// array
		...
		f = UGen.ar("RHPF",
				UGen.ar("*",
						UGen.ar("max",
								UGen.ar("MulAdd", UGen.array(UGen.ar("BrownNoise"), UGen.ar("BrownNoise")),
										UGen.ir(0.5f), UGen.ir(-0.49f)),
								UGen.ir(0)),
						UGen.ir(20)),
				UGen.ir(5000), UGen.ir(1));
		def = new SynthDef("JScratchy", UGen.ar("Out", UGen.ir(0), f));
		result.add(def);

		// Sprinkler
		f = UGen.ar("BPZ2",
				UGen.ar("*", UGen.ar("WhiteNoise"),
						UGen.kr("*",
								UGen.kr("LFPulse",
										UGen.kr("MulAdd",
												UGen.kr("LFPulse", UGen.ir(0.09f), UGen.ir(0), UGen.ir(0.16f)),
												UGen.ir(10), UGen.ir(7)),
										UGen.ir(0), UGen.ir(0.25f)),
								UGen.ir(0.1f))));
		...
		// HarmonicSwimming
		{
			...
			for (int i = 0; i < partials; i++) {
				f = UGen.kr("max", UGen.ir(0),
						UGen.kr("MulAdd",
								UGen.kr("LFNoise1",
										UGen.array(UGen.ir(2 + rnd.nextFloat() * 8), UGen.ir(2 + rnd.nextFloat() * 8))),
								UGen.ir(0.02f), offset));
		...
		// (which won't happend in neither language ; see also the orphan 'n')
		{
			...
			f = UGen.ar("Klank", UGen.ar("*", UGen.ar("Dust", UGen.ir(0.7f)), UGen.ir(0.04f)), UGen.ir(1.0f),
					UGen.ir(0.0f), UGen.ir(1.0f), new GraphElemArray(flop));
		...
		// in the sc sources, but it simply adds up all arguments
		{
			...
			for (int i = 0; i < num; i++) {
				g = UGen.ar("Pan2",
						UGen.ar("*", UGen.ar("SinOsc",
								UGen.kr("MulAdd",
										UGen.kr("SinOsc", UGen.ir(rnd.nextFloat() * 0.1f + 0.02f),
												UGen.ir(rnd.nextFloat() * 2 * (float) Math.PI)),
										UGen.ir(rnd.nextInt(600)), UGen.ir(rnd.nextInt(600) + 700)),
								UGen.ir(0)),
			...
			g = UGen.ar("*",
					...
					UGen.kr("MulAdd", UGen.kr("LFNoise2", UGen.array(UGen.ir(0.3f), UGen.ir(0.3f))), UGen.ir(0.15f),
							UGen.ir(0.18f)));
		...
		// SampleAndHoldLiquidities
		{
			...
			GraphElem freq = UGen.kr("Latch",
					UGen.kr("MulAdd", UGen.kr("WhiteNoise"), UGen.kr("*", centerFreq, UGen.ir(0.5f)), centerFreq),
					clock);
			GraphElem panPos = UGen.kr("Latch", UGen.kr("WhiteNoise"), clock);

			f = UGen.ar("*", UGen.ar("SinOsc", freq), UGen.kr("Decay2", clock, UGen.kr("*", UGen.ir(0.1f), clockTime),
					UGen.kr("*", UGen.ir(0.9f), clockTime)));
		...
		// AleatoricQuartet
		{
			...
			for (int i = 0; i < 4; i++) {
				excitation = UGen.ar("*", UGen.ar("PinkNoise"),
						UGen.kr("max", UGen.ir(0), UGen.kr("MulAdd", UGen.kr("LFNoise1", UGen.ir(8)), dmul, dadd)));

				freq = UGen.kr("midicps", UGen.kr(
						"Lag", UGen
								.kr("round",
										UGen.kr("MulAdd", UGen.kr("LFNoise0", UGen.ir(fRange[rnd.nextInt(3)])),
												UGen.ir(7), UGen.ir(36 + rnd.nextInt(60))),
										UGen.ir(1)),
						UGen.ir(0.2f)));

				g = UGen.ar("Pan2",
						UGen.ar("CombL", excitation, UGen.ir(0.02f), UGen.kr("reciprocal", freq), UGen.ir(3)),
						UGen.ir(rnd.nextFloat() * 2 - 1));
			...
			for (int i = 0; i < 5; i++) {
				f = UGen.ar("AllpassN", f, UGen.ir(0.05f),
						UGen.array(UGen.ir(rnd.nextFloat() * 0.05f), UGen.ir(rnd.nextFloat() * 0.05f)), UGen.ir(1));
			}
			...
			for (int i = 0; i < 4; i++) {
				g = UGen.ar("*", UGen.ar("LFSaw", UGen.kr("midicps",
						UGen.kr("MulAdd", UGen.kr("LFPulse", UGen.ir(0.06f), UGen.ir(0), UGen.ir(0.5f)), UGen.ir(2),
								UGen.array(UGen.ir(34 + rand2(rnd, 0.1f)), UGen.ir(34 + rand2(rnd, 0.1f)))))),
						UGen.ir(0.01f));
				f = f == null ? g : UGen.ar("+", f, g);
			}
			g = UGen.kr("MouseY", UGen.ir(0.1f), UGen.ir(0.7f));
			h = UGen.kr("LinExp", UGen.kr("SinOsc", UGen.ir(0.07f)), UGen.ir(-1), UGen.ir(1), UGen.ir(300),
					UGen.ir(5000));
		...
		// NoiseBusiness2
		{
			...
			g = UGen.ar("SinOsc", UGen.ir(exprand(rnd, 0.3f, 8)), UGen.ir(rnd.nextFloat() * pi2));
		...
		// NoiseBusiness4
		{
			...
			f = UGen.ar("*",
					UGen.ar("LFPulse", UGen.ir(80 * rrand(rnd, 1, 32)), UGen.ir(rnd.nextFloat() * pi2), UGen.ir(0.1f)),
					UGen.ar("*", f, h));
		...
		{
			Control kCtrl = Control.kr(new String[] { "out", "freq", "decay", "wet", "gate" },
					new float[] { 0, 400, 2, 1, 1 });
		...
		// RingMod
		{
			...
			GraphElem wet = UGen.kr("Ramp", kCtrl.getChannel("wet"), UGen.ir(0.1f));
			GraphElem zin = UGen.ar("In", 2, kCtrl.getChannel("out"));
			GraphElem zout = UGen.ar("*", zin,
					UGen.ar("SinOsc", freq, UGen.array(UGen.ir(0), UGen.ir((float) Math.PI / 2))));
