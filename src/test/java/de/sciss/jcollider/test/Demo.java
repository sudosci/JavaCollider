 ...
 * options. Before booting, check that the application path to scsynth is
 * correctly specified in the main frame.
 * <p>
 * Buttons at the bottom of the main frame will create a synth for the selected
 * def, stop all synths, open a synthdef diagram and print the def to the system
 * console. A tree view of the node graph can be created.
 *
 * @version 0.32, 25-Feb-08
 * @author Hanns Holger Rutz
 *
 * @todo the demo and the NodeWatcher get irritated if a server is already
 *       running and containing nodes when the demo is launched
 * @todo quitting the application from the apple menu will not perform
 *       clean-ups, notably the server will not be shut down. please use main
 *       window's close gadget for now
 * @todo should be possible to drag + drop nodes in the tree panel, create
 *       subgroups etc.
 */
public class Demo extends JFrame implements FileFilter, ServerListener, Constants {
	...
	public Demo() {
		...
		// pack();
		if (spf != null)
			setLocation(spf.getX() + spf.getWidth() + 24, spf.getY());
		setSize(512, 512);
		setVisible(true);
		toFront();
	}
		...
		for (int i = 0; i < defTables.length; i++) {
			...
			for (int j = 0; j < defs.size(); j++) {
				...
				try {
					...
					System.err.println("Sending Def " + def.getName() + " : " + e1.getClass().getName() + " : "
							+ e1.getLocalizedMessage());
				}
			}
		}
	...
	private class ActionDiagram extends AbstractAction {
	...
	private class ActionSynthDefApiEx extends AbstractAction {
	...
	private class ActionNodeTree extends AbstractAction {
